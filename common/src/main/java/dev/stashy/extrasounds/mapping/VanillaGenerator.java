...
import static dev.stashy.extrasounds.sound.Categories.*;
import static dev.stashy.extrasounds.sound.Sounds.*;
...
public class VanillaGenerator {
    ...
    static {
        ...
        map.put(BucketItem.class, it -> {
            ...
            if (f != null) {
                entry = f.getBucketFillSound().isPresent() ? event(f.getBucketFillSound().get().getId()) : null;
            }
